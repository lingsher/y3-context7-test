...
Y3 编辑器常量定义，包含单位属性、技能类型、UI事件等常量定义。
...
| 物理吸血 | 'vampire_phy' |
| 法术吸血 | 'vampire_mag' |
| 物理穿透比例 | 'pene_phy_ratio' |
| 法术穿透比例 | 'pene_mag_ratio' |
| 受伤减免 | 'dmg_reduction' |
| 伤害加成 | 'extra_dmg' |
| 被治疗加成 | 'healing_effect' |
| 移动速度 | 'ori_speed' |
| 真实视野 | 'vision_true' |
...
## y3.Const.DirectionalLightAttribute

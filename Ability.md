...
Y3 编辑器技能对象相关接口，用于获取和设置技能属性、冷却、层数等。
...
| [api_get_ability_index](#api_get_ability_index) | 获取技能序号 |
...
| [api_set_ability_build_id](#api_set_ability_build_id) | 设置技能的建造目标类型 |
...
| [api_add_tag](#api_add_tag) | 技能添加标签 |
| [api_remove_tag](#api_remove_tag) | 技能移除标签 |
...
```lua
local value = ability:api_get_str_attr("name")
```
...
```lua
ability:api_set_str_attr("desc", "这是一个强力技能")
```
...
```lua
local owner = ability:api_get_owner()
```
...
```lua
local owner_id = ability:api_get_owner_id()
```
...
```lua
local ability_type = ability:api_get_type()
```
...
```lua
local index = ability:api_get_ability_index()
```
...
```lua
local seq = ability:api_get_ability_seq()
```
...
```lua
local cast_type = ability:api_get_ability_cast_type()
```
...
```lua
local global_id = ability:api_get_ability_global_id()
```
...
```lua
local level = ability:api_get_level()
```
...
```lua
ability:api_remove_kv("custom_data")
```
...
```lua
local value = ability:api_calc_ability_formula_kv("damage_formula")
```
...
```lua
ability:add_timer(2.0, function()
    print("定时器触发")
end)
```
...
- 参数
    ...
    | 参数名 | 数据类型 | 说明 |
    | :--- | :--- | :--- |
    | runtime_id | integer? | runtime_id (可选) |
...
```lua
local has_target = ability:api_has_target()
```
...
```lua
local pos = ability:api_get_release_position()
```
...
```lua
local direction = ability:api_get_release_direction()
```
...
```lua
local damage = ability:api_get_float_attr("damage")
```
...
```lua
local count = ability:api_get_int_attr("hit_count")
```
...
```lua
local is_aoe = ability:api_get_bool_attr("is_aoe")
```
...
```lua
local range = ability:api_get_ability_cast_range()
```
...
```lua
ability:api_set_ability_cast_range(800)
```
...
```lua
ability:api_set_ability_build_rotate(90)
```
...
```lua
ability:api_set_ability_sector_radius(500, false)
```
...
```lua
ability:api_set_ability_sector_angle(60, false)
```
...
```lua
ability:api_set_ability_arrow_width(100, false)
```
...
```lua
ability:api_set_ability_arrow_length(800, false)
```
...
```lua
ability:api_set_ability_circle_radius(300, false)
```
...
```lua
ability:api_set_ability_pointer_type(1)
```
...
```lua
local pointer_type = ability:api_get_ability_skill_pointer()
```
...
```lua
ability:api_set_ability_icon(100001)
```
...
```lua
local cost = ability:api_get_ability_player_attr_cost("gold")
```
...
```lua
ability:api_set_ability_player_attr_cost("gold", 100)
```
...
```lua
ability:api_add_ability_player_attr_cost("gold", 50)
```
...
```lua
ability:api_add_float_attr("damage", 50.0)
```
...
```lua
ability:api_set_float_attr("damage", 100.0)
```
...
```lua
ability:api_add_int_attr("hit_count", 1)
```
...
```lua
ability:api_set_int_attr("hit_count", 5)
```
...
```lua
ability:api_set_bool_attr("is_aoe", true)
```
...
```lua
ability:api_break_ability_in_cs()
```
...
```lua
local level = ability:api_get_ability_nearest_upgradable_unit_level()
```
...
```lua
local ability_id = ability:api_get_ability_id()
```
...
```lua
local is_melee = ability:api_is_melee_ability()
```
...
```lua
local is_common = ability:api_is_common_atk()
```
...
```lua
local is_passive = ability:is_passive_ability()
```
...
```lua
local can_die = ability:api_get_cost_hp_can_die()
```
...
```lua
ability:api_set_cost_hp_can_die(false)
```
...
```lua
local can_cast = ability:api_get_can_cast_when_hp_insufficient()
```
...
```lua
ability:api_set_can_cast_when_hp_insufficient(true)
```
...
```lua
local name = ability:api_get_name()
```
...
```lua
ability:api_set_influenced_by_cd_reduce(true)
```
...
```lua
local influenced = ability:api_get_influenced_by_cd_reduce()
```
...
```lua
local stack = ability:api_get_ability_stack()
```
...
```lua
ability:api_add_ability_stack_count(1)
```
...
```lua
ability:api_set_ability_stack_count(3)
```
...
```lua
local cd_left = ability:api_get_cd_left_time()
```
...
```lua
ability:api_immediately_clear_cd()
```
...
```lua
ability:api_change_ability_cd_cold_down(5.0)
```
...
```lua
ability:api_set_ability_cd(10.0)
```
...
```lua
ability:api_add_ability_cd(2.0)
```
...
```lua
ability:api_set_ability_cur_stack_cd(3.0)
```
...
```lua
local stack_cd = ability:api_get_stack_cd_left_time()
```
...
```lua
ability:api_play_sound_by_ability_for_role_relation(camp_target, 100001, true)
```
...
```lua
ability:api_set_autocast_enabled(true)
```
...
```lua
local is_auto = ability:api_is_autocast_enabled()
```
...
```lua
ability:api_set_ability_build_id(134217729)
```
...
```lua
ability:api_get_ability_build_id()
```
...
```lua
ability:api_set_ability_build_area(area)
```
...
```lua
local item = ability:api_get_item()
```
...
```lua
ability:api_set_ability_is_permanent(true)
```
...
```lua
ability:api_add_filter_unit_tag("敌人")
```
...
```lua
ability:api_remove_filter_unit_tag("敌人")
```
...
```lua
ability:api_add_filter_item_tag("消耗品")
```
...
```lua
ability:api_remove_filter_item_tag("消耗品")
```
...
```lua
ability:api_add_collection_destructible_tags("可破坏")
```
...
```lua
ability:api_remove_collection_destructible_tags("可破坏")
```
...
```lua
ability:api_ability_set_stash(true)
```
...
```lua
local can_filter = ability:api_can_be_filtered_by_ability(target_unit)
```
...
```lua
ability:api_ability_fast_forward_ability_state()
```
...
```lua
local is_forbidden = ability:api_get_is_ability_forbidden()
```
...
```lua
ability:api_set_ability_pointer_transition_start_time(0.5)
```
...
```lua
ability:api_set_ability_pointer_transition_duration(0.3)
```
...
```lua
local charge_time = ability:api_get_charge_time()
```
